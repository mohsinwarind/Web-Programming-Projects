...
<div style="background: white; padding: 25px; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.08);">
    <form method="PUT" action="{{ route('admin.category.update', $category->id) }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-3">
            ...
            @if($category->image)
                <div class="mb-2">
                    <img src="{{ $category->image }}" alt="{{ $category->name }}" style="max-height: 150px; border-radius: 5px;">
                </div>
            @endif
